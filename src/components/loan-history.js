class LoanHistory extends HTMLElement {
  constructor() {
    super();
  ...
  renderHistory(calculations) {
    ...
    if (calculations.length === 0) {
      historyList.innerHTML = '<li class="list-group-item">No se encontraron cálculos anteriores.</li>';
      return;
    }
    calculations.forEach(calc => {
      ...
      listItem.innerHTML = `
        <div>
          <strong>${new Date(calc.date).toLocaleString()}</strong> - Monto: ${calc.amount}, Tasa: ${calc.rate}%, Periodos: ${calc.periods}
        </div>
        <button class="btn btn-sm btn-info" data-id="${calc.id}">Ver Detalle</button>
      `;
      listItem.querySelector('button').addEventListener('click', async () => {
        try {
          const res = await fetch(`http://localhost:3000/calculations/${calc.id}`);
          if (!res.ok) throw new Error('Error al cargar el detalle');
          const calcDetail = await res.json();
          // Despachar evento para que el componente de la tabla muestre el detalle
          window.dispatchEvent(new CustomEvent('loanHistoryDetail', { detail: { schedule: calcDetail.schedule } }));
        } catch (error) {
...
customElements.define('loan-history', LoanHistory); 