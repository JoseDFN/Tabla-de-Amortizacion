class LoanCalculatorForm extends HTMLElement {
  constructor() {
    super();
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="container mt-3">
        <form id="loan-form">
          ...
          <button type="submit" class="btn btn-primary">Generar Tabla de Amortización</button>
        </form>
      </div>
    `;
  }
  ...
  // Método para calcular la amortización alemana (cuota de amortización fija)
  calculateAmortizationAlemana(amount, rate, periods) {
    const principal = amount / periods;
    let remainingBalance = amount;
    const schedule = [];
    const monthlyRate = rate / 100 / 12;
    for (let i = 1; i <= periods; i++) {
      const saldoInicial = remainingBalance;
      const intereses = saldoInicial * monthlyRate;
      const cuota = principal + intereses;
      ...
      schedule.push({
        ...
        saldoRestante: remainingBalance.toFixed(2)
      });
    }
    return schedule;
  }
  ...
  async handleSubmit(event) {
    ...
    } else if (amortizationType === 'alemana') {
      schedule = this.calculateAmortizationAlemana(amount, rate, periods);
    } else if (amortizationType === 'americana') {
      schedule = this.calculateAmortizationAmericana(amount, rate, periods);
    }
    
    // Despachar un evento global para actualizar el componente de la tabla
    window.dispatchEvent(new CustomEvent('loanCalculationDone', { detail: { schedule } }));
...
customElements.define('loan-calculator-form', LoanCalculatorForm); 